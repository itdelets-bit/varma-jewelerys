...
function showUndoBarForDelete(product){
const bar = getEl("undoBar");
...
function setReportSummary({ bills = 0, qty = 0, amount = 0, lowStock = 0 }){
...
setReportSummary({ bills: 0, qty: 0, amount: 0, lowStock: lowRows.length });
return;
}
...
setReportSummary({ bills: totalBills, qty: totalQty, amount: totalAmount, lowStock: lowStockCount });
return;
}

setReportHead(["Date", "Customer", "Product", "Size", "Qty", "Rate", "Amount"]);
let totalQty = 0;
let totalAmount = 0;
...
const items = Array.isArray(sale.data.items) ? sale.data.items : [];
if(items.length === 0){
rows.push(`<tr><td>${formatDisplayDate(sale.saleDateISO)}</td><td>${sale.saleCustomer || "-"}</td><td>-</td><td>-</td><td>0</td><td>0</td><td>${Number(sale.data.total) || 0}</td></tr>`);
...
totalQty += qty;
totalAmount += amount;
rows.push(`<tr><td>${formatDisplayDate(sale.saleDateISO)}</td><td>${sale.saleCustomer || "-"}</td><td>${parsed.product}</td><td>${parsed.size}</td><td>${qty}</td><td>${rate}</td><td>${amount}</td></tr>`);
});
});
